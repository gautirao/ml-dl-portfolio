...
class GroundedCitation(BaseModel):
    ...
    citation_label: str = Field(..., description="The human-readable label for the source (e.g., 'NatWest T&Cs, p. 5').")
    text_segment: str = Field(..., description="The specific segment of text from the chunk being cited.")
    page_number: int | None = Field(None, description="The page number if available in metadata.")
    section_heading: str | None = Field(None, description="The section heading if available in metadata.")

class GroundedAnswer(BaseModel):
    """The final domain model for a grounded answer."""
    decision: GroundedAnswerDecision
    answer: str | None = Field(None, description="The generated answer text. Null if decision is not ANSWER.")
    citations: list[GroundedCitation] = Field(default_factory=list)
    limitations: list[str] = Field(default_factory=list, description="Any limitations or caveats regarding the answer.")
    evidence_chunk_ids_used: list[str] = Field(default_factory=list, description="Deduplicated list of chunk IDs actually used.")
    model: str
