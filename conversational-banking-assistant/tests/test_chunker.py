...
from cba.models import ExtractedDocument, ExtractedPage, DocumentType, ProductArea, Chunk
from cba.chunker import TextCleaner, SectionDetector, SectionAwareChunker
...
def test_text_cleaner_removes_boilerplate():
    cleaner = TextCleaner()
...
def test_section_detector_identifies_numbered_headings():
...
def test_section_detector_identifies_uppercase_headings():
    detector = SectionDetector()
    assert detector.is_heading("OVERDRAFTS") is True
    assert detector.is_heading("IMPORTANT INFORMATION") is True

def test_section_detector_ignores_currency_and_rates():
    detector = SectionDetector()
...
def test_chunks_never_cross_sections(mock_extracted_doc):
...
def test_overlap_resets_at_section_boundary():
...
def test_offsets_match_cleaned_text(mock_extracted_doc):
...
def test_page_number_start_is_preserved(mock_extracted_doc):
